...
class App:
    ...
    def __init__(self, master):
        ...
        self.config = {'directories': ['Academics', 'Cultural', 'Sports', 'Technical', 'Hostel5'],
                       ...
                       # fill,font(face,size)
                       'tickerstyle': ['pink', ("Helvectica", "24")],
                       'canvasbg': 'red',
                       'tickerrectcolor': 'pink',
                       'picspeed': 2000,  # Switch images every x seconds
                       'picsatatime': 4,  # 1,2, or 4
                       'tilingpad': [2, 2],  # horiz,vert
                       # After how many iterations ought I refresh?
                       # [PAT1,PAT2,PAT3]
                       'refreshcount': {1: 10, 2: 10, 4: 10},
                       # Central canvas margins, [[left,right],[top,bottom]]
                       'mainpad': [[1, 1], [50, 1]],
                       'logoscale': [200, 40]
                       }
        try:
        ...
        self.can = Canvas(self.rt, bg="black", borderwidth=0,
                          highlightthickness=0, background=self.config['canvasbg'])
        ...
        self.getpiclist()
        self.tickerrect = self.can.create_rectangle(0, self.imgbbox[1] + self.config['mainpad'][1][0] + self.config[
                                                    'mainpad'][1][1], self.windowd[0], self.windowd[1], fill=self.config['tickerrectcolor'])
        ...
        if self.config['picsatatime'] == 1:
            self.picindex = [0]
            self.imgs = [self.can.create_image(self.imgbbox[0] / 2 + self.config['mainpad'][0][0], self.imgbbox[
                                               1] / 2 + self.config['mainpad'][1][0], image=self.photos[0], anchor=CENTER)]
        elif self.config['picsatatime'] == 2:
            self.picindex = [0, 0]

            self.imgs = [0, 0]
            self.imgs[0] = self.can.create_image(self.imgbbox[0] / 4 - self.config['tilingpad'][0], self.imgbbox[
                                                 1] / 2, image=self.photos[self.config['directories'][0]][self.picindex[0]], anchor=CENTER)
            self.imgs[1] = self.can.create_image(3 * self.imgbbox[0] / 4 + self.config['tilingpad'][0], self.imgbbox[
                                                 1] / 2, image=self.photos[self.config['directories'][1]][self.picindex[1]], anchor=CENTER)
        elif self.config['picsatatime'] == 4:
            self.picindex = [0, 0, 0, 0]

            self.imgs = [0, 0, 0, 0]

            self.imgs[0] = self.can.create_image(self.imgbbox[0] / 4 - self.config['tilingpad'][0] / 2, self.imgbbox[1] / 4 - self.config[
                                                 'tilingpad'][1] / 2, image=self.photos[self.config['directories'][0]][self.picindex[0]], anchor=CENTER)
            self.imgs[1] = self.can.create_image(3 * self.imgbbox[0] / 4 + self.config['tilingpad'][0] / 2, self.imgbbox[1] / 4 - self.config[
                                                 'tilingpad'][1] / 2, image=self.photos[self.config['directories'][1]][self.picindex[1]], anchor=CENTER)
            self.imgs[2] = self.can.create_image(self.imgbbox[0] / 4 - self.config['tilingpad'][0] / 2, 3 * self.imgbbox[1] / 4 + self.config[
                                                 'tilingpad'][1] / 2, image=self.photos[self.config['directories'][2]][self.picindex[2]], anchor=CENTER)
            self.imgs[3] = self.can.create_image(3 * self.imgbbox[0] / 4 + self.config['tilingpad'][0] / 2, 3 * self.imgbbox[1] / 4 + self.config[
                                                 'tilingpad'][1] / 2, image=self.photos[self.config['directories'][3]][self.picindex[3]], anchor=CENTER)
        self.loadLogo()
        self.updLogo()
    ...
    def moveticker(self):
        ...
        if self.tickerstate is 1:
            ...
            self.getticker()
            self.tickerstate = 1
            self._tickertext = self.can.create_text(self.can.canvasx(0), self.can.canvasy(0), text=self.config[
                                                    'delimiter'] + self.tickerlist, fill=self.config['tickerstyle'][0], font=self.config['tickerstyle'][1])
            a = self.can.bbox(self._tickertext)
            self.can.move(self._tickertext, self.windowd[
                          0] - a[0], self.windowd[1] + a[1] - self.config['tickerpad'][0])
    ...
    def movepic(self):
        ...
        if self.config['picsatatime'] == 1:
            ...
            if self.picindex[0] >= len(self.piclist):
                self.picindex = [0]
            self.imgs[0] = self.can.create_image(self.imgbbox[0] / 2 + self.config['mainpad'][0][0], self.imgbbox[
                                                 1] / 2 + self.config['mainpad'][1][0], image=self.photos[self.picindex[0]], anchor=CENTER)
        elif self.config['picsatatime'] == 2:
            for i in range(0, 2):
                self.picindex[i] += 1
    ...
    def getticker(self):
        ...
        try:
            for i in self.config['directories']:
                #print [os.listdir(i)]
                temp += [open(os.path.join(i, f)).read()
                         for f in os.listdir(i) if f.endswith('.txt')]
    ...
    def getpiclist(self):
        ...
        try:
            for i in self.config['directories']:
                #print [os.listdir(i)]
                t2[i] = [os.path.join(i, f) for f in os.listdir(i) if (
                    f.endswith('.png') or f.endswith('.jpg') or f.endswith('.gif'))]
                temp += t2[i]
            x = False
            ...
            if len(self.config['directories']) >= 4 and self.config['picsatatime'] == 4:
                for i3 in range(4, len(self.config['directories'])):
                    t2[self.config["directories"][i3 - 4]
                       ] += t2[self.config["directories"][i3]]
            ...
            if t2 != {} and not x:
                ...
                if self.config['picsatatime'] == 1:
                    ...
                    for i in range(0, len(temp)):
                        ...
                        self.photos[i].thumbnail(
                            (self.imgbbox[0], self.imgbbox[1]))
                    ...
                    for i in self.config['directories']:
                        ...
                        for j in range(0, len(t2[i])):
                            ...
                            self.photos[i][j].thumbnail(
                                (self.imgbbox[0] / 2 - self.config['tilingpad'][0], self.imgbbox[1]))
                            self.photos[i][j] = ImageTk.PhotoImage(
                                self.photos[i][j])
                elif self.config['picsatatime'] == 4:
                    self.photos = {}
                    ctr = 0
                    for i in self.config['directories']:
                        ...
                        for j in range(0, len(t2[i])):
                            ...
                            self.photos[i2][j + jd].thumbnail((self.imgbbox[0] / 2 - self.config['tilingpad'][
    ...
    def updLogo(self):
        self.logoC = self.can.create_image(
            self.windowd[0], 0, image=self.logo, anchor=NE)

    def loadLogo(self):
        self.logo = Image.open("logo.png")
        self.logo.thumbnail(
            (self.config['logoscale'][0], self.config['logoscale'][0]))
